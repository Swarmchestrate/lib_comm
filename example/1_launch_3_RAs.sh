...
NUM_NODES=3             # Number of nodes to start
NODE_IDS=("wmin.ac.uk" "napier.ac.uk" "sztaki.hu")
...
start_node() {
    ...
    if [ -z "${join_port}" ]; then
        ...
        ( poetry run python3 example_ra.py \
            --listen 127.0.0.1:${port} --public 127.0.0.1:${port} \
            --id "${NODE_IDS[${node_num}]}" 2>&1 ) | \
            sed "s/^/node${node_num}: /" &
    else
        ...
        # Run in a subshell to capture the PID of the 'poetry run' process
        ( poetry run python3 example_ra.py --listen 127.0.0.1:${port} --public 127.0.0.1:${port} \
            --join 127.0.0.1:${join_port} --id "${NODE_IDS[${node_num}]}" 2>&1 ) | \
            sed "s/^/node${node_num}: /" &
    fi
}
...
PORTS+=("${FIRST_PORT}")
start_node 0 "${FIRST_PORT}" ""  # Pass an empty string as the third argument
sleep "${SLEEP_TIME}"
...
for ((i=1; i<NUM_NODES; i++)); do
    ...
    LATEST_INDEX=$((i - 1))  # Zero-based index
