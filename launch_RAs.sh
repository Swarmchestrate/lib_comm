...
start_node() {
    ...
    if [ -z "${join_port}" ]; then
        ...
        ( poetry run python3 example_ra.py --listen 127.0.0.1:${port} --public 127.0.0.1:${port} 2>&1 ) | \
            sed "s/^/node${node_num}: /" &
    else
        ...
        # Run in a subshell to capture the PID of the 'poetry run' process
        ( poetry run python3 example_ra.py --listen 127.0.0.1:${port} --public 127.0.0.1:${port} --join 127.0.0.1:${join_port} 2>&1 ) | \
            sed "s/^/node${node_num}: /" &
    fi
}
